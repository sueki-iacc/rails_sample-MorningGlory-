class Article < ActiveRecord::Base
  attr_accessible :title, :body, :released_at, :expired_at, :member_only, :no_expiration, as: :admin


	scope :member_only, where(member_only: true)
	scope :readable_for, ->(member){now=Time.current
	   rel=where(
		"released_at<=? AND (?<expired_at OR "+
			"expired_at IS NULL)",
		now, now)
